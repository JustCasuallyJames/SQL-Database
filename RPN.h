...
public:
    ...
    vector<long> get_greater_equal(string sfields, string svalues);
    //gets all the values that are greater
    vector<long> get_greater(string sfields, string svalues);
    //gets all the values that are less than
    vector<long> get_less(string sfields, string svalues);
    //gets all the values that are less than or equal
    vector<long> get_less_equal(string sfields, string svalues);
    //gets the value that is equal to the fields and the value
    vector<long> get_equal(string sfields, string svalues);
...
//gets ALL of the values that are greater than and equal
template<typename T>
vector<long> RPN<T>::get_greater_equal(string sfields, string svalues) {
    vector<long> results;
    for (MMap<string,long>::Iterator it = _indices[sfields].lower_bound(svalues); it != _indices[sfields].end(); it++) {
...
//gets all the values that are greater
template<typename T>
vector<long> RPN<T>::get_greater(string sfields, string svalues) {
    vector<long> results;
    for (MMap<string, long>::Iterator it = _indices[sfields].upper_bound(svalues); it != _indices[sfields].end(); it++) {
        for (int i = 0; i < it->size(); i++) { // i< it->size() is the vector size of the number of records
            results.push_back(it->at(i)); //this grabs the values within the vector<long> to be pushed into our results vector
        }
    }
